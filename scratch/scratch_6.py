...
class Model_EDF(pynlo.model.Model):
    ...
    def simulate(self, z_grid, local_error=1e-6, n_records=None, plot=None):
        ...
        n5_n[0] = self.mode.n5
        na_n = np.empty(n_records, dtype=float)
        na_n[0] = self.mode.na
        nb_n = np.empty(n_records, dtype=float)
        nb_n[0] = self.mode.nb
        ...
        for z_stop in z_grid[1:]:
            ...
            if z in z_record:
                ...
                n1_n[idx] = self.mode.n1
                n2_n[idx] = self.mode.n2
                n3_n[idx] = self.mode.n3
                n4_n[idx] = self.mode.n4
                n5_n[idx] = self.mode.n5
                na_n[idx] = self.mode.na
                nb_n[idx] = self.mode.nb
